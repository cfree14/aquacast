...
for(i in 1:length(cost_scalars)){
  ...
  faq_orig <- readRDS(file.path(datadir, paste0("finfish_output_", suffix, ".Rds")))
  baq_orig <- readRDS(file.path(datadir, paste0("bivalve_output_", suffix, ".Rds")))
  ...
  # Export
  outfile <- paste0("global_capture_mariculture_output_merged_", suffix, ".Rds")
  saveRDS(gdata, file=file.path(datadir, outfile))
