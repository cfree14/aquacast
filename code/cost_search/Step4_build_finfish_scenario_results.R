...
for(i in 1:length(cost_scalars)){
  ...
  output <- purrr::map_df(1:nrow(scen_key), function(x) {
    ...
    # Expand mariculture
    out_df <- expand_mariculture(rcp=rcp_do, mgmt_scenario = mgmt_do, feed_scenario = feed_do, dev_scenario=dev_do, suffix=suffix)
    
  })
  
  # Export output
  outfile <- paste0("finfish_output_", suffix, ".Rds")
