...
inputdir <- "/Volumes/GoogleDrive/Shared drives/emlab/projects/current-projects/blue-paper-2/data/output/cost_search"
...
rcp <- "rcp85"; type <- "bivalve"; outdir <- outputdir; suffix <- "cost_search_1.3"
...
for(i in 1:length(cost_scalars)){
  ...
  # Bivalves
  merge_results(rcp="RCP26", type="bivalve", outdir=outputdir, suffix=suffix)
  merge_results(rcp="RCP45", type="bivalve", outdir=outputdir, suffix=suffix)
  merge_results(rcp="RCP60", type="bivalve", outdir=outputdir, suffix=suffix)
  merge_results(rcp="RCP85", type="bivalve", outdir=outputdir, suffix=suffix)
  
  # Finfish - 11 minutes each
  merge_results(rcp="RCP26", type="finfish", outdir=outputdir, suffix=suffix)
  merge_results(rcp="RCP45", type="finfish", outdir=outputdir, suffix=suffix)
  merge_results(rcp="RCP60", type="finfish", outdir=outputdir, suffix=suffix)
  merge_results(rcp="RCP85", type="finfish", outdir=outputdir, suffix=suffix)
