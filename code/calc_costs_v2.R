...
calc_cap_costs <- function(farm_design, harvest_yr){
  ...
  if(farm_design$type=="finfish"){
    ...
    juv_per_farm <- farm_design$nstocked_adj
...
calc_costs_v2 <- function(farm_design, cell_prod_mt_yr, fcr, vcells, harvest_yr){
  ...
  if(sum(suit_ncells!=cost_ncells)!=0){
    warning("The cost raster does not have the same number of cells as the viable raster.")
  }
