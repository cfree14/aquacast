...
# A few tests
out1 <- expand_mariculture(rcp="RCP 2.6", dev_scenario="Rational")
out2 <- expand_mariculture(rcp="RCP 2.6", dev_scenario="Current")
out3 <- expand_mariculture(rcp="RCP 2.6", dev_scenario="Proportional")
out4 <- expand_mariculture(rcp="RCP 2.6", dev_scenario="Need-based")
...
output <- purrr::map_df(1:nrow(scen_key), function(x) {
  ...
  # Expand mariculture
  out_df <- expand_mariculture(rcp=rcp_do, dev_scenario=dev_do)
  
})

# Export output
saveRDS(output, file=file.path(datadir, "bivalve_output.Rds"))
