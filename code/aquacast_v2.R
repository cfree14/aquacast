...
aquacast <- function(species, years=c(2021,2051,2100), rcp, outdir=F, plot=T){
  ...
  if(type=="Bivalve"){
    farm_design <- tibble(type="bivalve",
      ...
      mutate(line_ft=measurements::conv_unit(line_m, "m", "ft"),
             nstocked = nlines * line_ft * (harvest_cm_ft / harvest_cm))
  }
  ...
  data_df <- as.data.frame(cell_cost_usd_yr, xy=T) %>% 
    ...
    select(year, x, y, viable, prod_mt_yr, revenue_usd_yr, cost_usd_yr, profits_usd_yr)
  ...
  if(plot==T){
    ...
    # Plot
    g <- ggplot(stats, aes(x=year, y=prod_mt_yr_mil)) +
      geom_bar(stat="identity") +
      labs(x="", y="Production (millions of mt)", title=species$comm_name) +
      theme_bw() +
